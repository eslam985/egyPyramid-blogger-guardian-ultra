...
def authenticate(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    if credentials.username != correct_email or credentials.password != correct_password:
        raise HTTPException(
...
async def index(request: Request, user: str = Depends(authenticate), search: str = None, cat: str = None):
...
async def reset_sync(ep_id: int, user: str = Depends(authenticate)):
    try:
        ...
        supabase.table("episodes").update({"is_synced": False}).eq("id", ep_id).execute()
        return {"status": "success"} # FastAPI بيحول الديكشنري لـ JSON تلقائياً مش محتاج json.dumps
...
async def get_media_details(media_id: int, user: str = Depends(authenticate)):
    try:
        # جلب بيانات الميديا
        media_res = SupabaseService.client.table("medias").select("*").eq("id", media_id).single().execute()
        # جلب الحلقات المرتبطة بها مرتبة برقم الحلقة
        episodes_res = SupabaseService.client.table("episodes").select("*").eq("media_id", media_id).order("episode_number").execute()
